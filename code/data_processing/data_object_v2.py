...
class MultiTaskContrastiveDataset(data.Dataset):
    ...
    def get_label_matrix(self, drug_indices, gene_indices, type = "drug-protein"):
        ...
        if type == "drug-protein":
            return self.drug_protein_weights[drug_indices, :][:, gene_indices]
        elif type == "drug":
            return self.drug_weights[drug_indices, :][:, drug_indices]
        elif type == "protein":
            return self.protein_weights[gene_indices, :][:, gene_indices]
        else:
    ...
    def create_drug_weights(self, upper_drug_threshold = 0.7, lower_drug_threshold = 0.6, upper_target_threshold = 0.7, lower_target_threshold = 0.6):
        ...
        max_target_sim = torch.zeros(n_drugs, n_drugs)  # (num_drugs, num_drugs)
        for i in range(n_drugs):
            targets_i = self.drug_target_mat[i]  # (n_proteins,)
            # max protein sim from drug i's targets to each protein
            sim_to_prots = (self.protein_similarity_adj * targets_i.unsqueeze(1)).max(dim=0).values  # (n_proteins,)
            # max over drug j's targets
            max_target_sim[i] = (self.drug_target_mat * sim_to_prots.unsqueeze(0)).max(dim=1).values  # (n_drugs,)
    ...
    def create_protein_weights(self, upper_protein_threshold = 0.7, lower_protein_threshold = 0.6, upper_drug_threshold = 0.7, lower_drug_threshold = 0.6):
        ...
        max_drug_sim = torch.zeros(n_proteins, n_proteins)
        for i in range(n_proteins):
            drugs_i = target_drug_mat[i]  # (n_drugs,)
            sim_to_drugs = (self.drug_similarity_adj * drugs_i.unsqueeze(1)).max(dim=0).values  # (n_drugs,)
            max_drug_sim[i] = (target_drug_mat * sim_to_drugs.unsqueeze(0)).max(dim=1).values  # (n_proteins,)
    ...
    def __getitem__(self, idx):
        ...
        drug_embedding = self.smiles_to_embedding[drug].float()
        gene_embedding = self.uniprot_to_embedding[gene].float()
        drug_index = torch.tensor(self.smiles_to_index[drug], dtype=torch.long)
        gene_index = torch.tensor(self.uniprot_to_index[gene], dtype=torch.long)
