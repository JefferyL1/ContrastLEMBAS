"""
MoLFormer-c3-1.1B Embedding Generator
Generates molecular embeddings from SMILES strings.

Usage:
    # From a CSV file:
    python molformer_embeddings.py --input molecules.csv --smiles_col smiles --output embeddings.pkl

    # With a label column (saved alongside embeddings):
    python molformer_embeddings.py --input molecules.csv --smiles_col smiles --label_col activity --output embeddings.pkl
"""
...
def generate_embeddings(smiles_list, tokenizer, model, device, batch_size=64, max_length=512, pooling="cls"):
    """
    Generate embeddings for a list of SMILES strings.
    
    pooling options:
        "cls"  — CLS token (position 0)
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Generating embeddings"):
            # Grab the SMILES strings corresponding to this batch
            batch_smiles = [next(smiles_iter) for _ in range(len(batch["input_ids"]))]

            batch = {k: v.to(device) for k, v in batch.items()}

            outputs = model.molformer(**batch)
            ...
            # Store each SMILES -> tensor (moved to CPU)
            for smi, emb in zip(batch_smiles, batch_embeddings):
...
def main():
    parser = argparse.ArgumentParser(description="Generate MoLFormer-c3-1.1B embeddings from SMILES.")
    ...
    parser.add_argument("--pooling", type=str, default="cls", choices=["cls", "mean"], help="Pooling strategy: cls or mean.")
